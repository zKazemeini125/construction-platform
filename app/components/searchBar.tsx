        ...
        {results.map((item, rowIndex) => (
          <tr
            ...
            className={`cursor-pointer border-b border-(--muted) last:border-b-0 transition-colors ${
              rowIndex === activeIndex ? "bg-(--muted)" : "hover:bg-(--muted)/60"
            }`}
          >
            {columns.map((col) => (
    ...
    <>
      ...
      <div ref={containerRef} className={`relative hidden sm:block ${className}`}>
        <div className="flex items-center overflow-hidden rounded-lg bg-(--muted)">
          <Search size={16} strokeWidth={1.8} className="mx-2.5 shrink-0 text-(--muted-foreground)" />
          <input
            ref={desktopInputRef}
            type="text"
            role="combobox"
        ...
        <div className="fixed inset-0 z-100 flex flex-col bg-black/40 sm:hidden">
          <div className="flex max-h-[85vh] flex-col rounded-b-xl bg-(--background) shadow-(--shadow-header)">
            <div className="flex items-center gap-2 border-b border-(--muted) p-3">
              <Search size={18} strokeWidth={1.8} className="shrink-0 text-(--muted-foreground)" />
              <input
                ref={modalInputRef}
                type="text"
                placeholder={placeholder}
          ...
          <div className="flex-1" onClick={() => setIsMobileModalOpen(false)} aria-hidden="true" />
        </div>
      )}
    </>
  );
